...
def rpm_data(s, ch, a):
    ...
    if not (0 <= a.quality_reject_ratio <= 1): raise DSLError("--quality-reject-ratio must be between 0 and 1")
    i, name = s.resolve(ch); st, en = span(s, a.start_s, a.end_s); cur = s.bit(i, st); prev = None
    out = []; mindt = a.min_edge_spacing_ms*1e-3; selected = short_reject = sanity_reject = 0
    for n, new in s.rows(i, max(1, st+1), en):
        ...
        if dt <= 0: continue
        if mindt > 0 and dt < mindt:
            short_reject += 1; continue
        f = 1/dt; r = 60*f/a.ppr
        if a.rpm_sanity_max > 0 and r > a.rpm_sanity_max:
            sanity_reject += 1; continue
        out.append((n/s.hz, r, dt, f))

    considered = len(out) + short_reject + sanity_reject
    rejected = short_reject + sanity_reject
    reject_ratio = rejected/considered if considered else 1.0
    quality = dict(selected_edges=selected, valid_samples=len(out), rejected_short=short_reject,
                   rejected_sanity=sanity_reject, rejected_ratio=reject_ratio)

    bad = len(out) < a.min_valid_rpm_samples or (considered >= a.min_valid_rpm_samples and reject_ratio >= a.quality_reject_ratio)
    if bad:
        msg = (f"RPM signal quality check failed on CH{i} ({name}): valid={len(out):,}, "
               f"rejected={rejected:,}/{considered:,} ({100*reject_ratio:.4f}%). Check channel mapping, "
               "sensor output, edge polarity, PPR, and filters. Use --allow-low-quality-rpm only for debugging.")
        if not a.allow_low_quality_rpm: raise DSLError(msg)
        print(f"WARNING: {msg}", file=sys.stderr)
    elif a.min_edge_spacing_ms == 0 and a.rpm_sanity_max == 0:
        print("NOTE: No RPM quality limits are active; signal-model compatibility is not being sanity-checked.", file=sys.stderr)

    if not out: raise DSLError("No valid RPM samples; check channel/edge/PPR/filters")
    return (i, name), medf(out, a.rpm_median_window), quality
...
def parser():
    ...
    p.add_argument("--quality-reject-ratio", type=float, default=.99); p.add_argument("--min-valid-rpm-samples", type=int, default=3); p.add_argument("--allow-low-quality-rpm", action="store_true")
...
def main():
    ...
    try:
        with DSL(a.dsl) as s:
            th, seg, step = throttle(s, a.throttle_channel, a); rp, rows, quality = rpm_data(s, a.rpm_channel, a); m = metrics(rows, step, a); out = Path(a.output); prefix = Path(a.output_prefix) if a.output_prefix else out.with_suffix("")
            payload = dict(version=VERSION, capture=str(Path(a.dsl)), sample_rate_hz=s.hz, throttle=dict(channel=th[0], name=th[1], selected_step=step, segments=seg), rpm=dict(channel=rp[0], name=rp[1], edge=a.rpm_edge, ppr=a.ppr, quality=quality), metrics=m)
            save(prefix, seg, rows, payload); report(out, Path(a.dsl).name, th, rp, rows, seg, step, m, a)
